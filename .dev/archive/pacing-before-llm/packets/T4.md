# Executor packet — T4 (`pacing-before-llm`)
...
**T2:** Pre-LLM pacing reject shape and pipeline ordering per §2 and `.dev/decision-logs/pacing-before-llm-T2.md` (when landed).

**Doc edits (minimum):**

- **`.dev/eval-strategy.md` §4:** Split pacing stratification:
  - **Post-LLM pacing reject** — `discard_reason=pacing_gate` **with** `event_reactor_results` row (high self-score never aired).
  - **Pre-LLM pacing reject** — `discard_reason=pacing_gate`, **no** reactor child, `llm_latency_ms` null; not “high score never aired”.
- **`README.md`:** One paragraph: pacing can skip LLM during cooldown; override does not apply on that path.
- **`CHANGELOG.MD`:** Entry under current version section.
- **gui-T1 cross-ref:** Add note in T2 decision log or README that live GUI preview does not fire on pre-LLM pacing skip (gui-T1 still valid for post-LLM paths).

**Retired-string sweep:** No references to `.dev/plans/_pending/pacing-before-llm/` in committed docs.
