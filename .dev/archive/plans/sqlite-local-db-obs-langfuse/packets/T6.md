...
No §5.2 tuple lists **T6**; optional import scope is encoded in **§3 Subtask T6 kill criteria** on this packet.

---

## 5. Filtered hidden couplings (§5.4)

No §5.4 tuple lists **T6** exclusively.

---

## 6. Resolved inputs

- **T3:** to be supplied at execution time: stable **`insert_*`** / **`HecklerLogger`** persistence path into **`events`** table for reuse by the import script.
