...
Follow **`executor-subtask-execution`** (`SKILL.md`). Log tier: **architectural**. Decision log: **`.dev/decision-logs/T13.md`**.
...
No §5.2 tuple lists **T2** in the subtask ID position; config paths participate indirectly in cross-cutting assumptions owned by **T1**, **T3**, **T4**.
...
- None required before execution; coordinate ordering with **T1** only if import cycles appear when running full suite mid-flight.
