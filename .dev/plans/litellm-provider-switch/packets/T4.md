...
Only items listing **T4** in the subtask ID tuple:
...
- **T1:** to be supplied at execution time: final env key names and `load_config` behavior as merged to `main` (or working branch), plus `T10.md`.
- **T2:** to be supplied at execution time: shipped `heckler/reactor.py` and `T11.md` for accurate documentation of behavior and log strings.
