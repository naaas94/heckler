...
Follow **`executor-subtask-execution`** (`SKILL.md`). Log tier: **architectural**. Decision log: **`.dev/decision-logs/T15.md`**.
...
- `(litellm tracing + SQLite correlation | §2 tracing_context + events DDL | orphan spans or duplicate trace roots if second instrumentation layer added | T4)` — **suspected**
...
- **T1:** to be supplied at execution time: **`heckler/tracing_context.py`** API (set/get/clear) aligned with **T12**.
- **T2:** to be supplied at execution time: **`HecklerConfig`** as consumed by **`Reactor`** (unchanged constructor aside from any ambient env dotenv behavior).
